...
export default function BabyProfile() {
  ...
  useEffect(() => {
    const fetchBaby = async () => {
      try {
        const response = await axios.get(`/api/Child/get-by-id/${childId}`);
  ...
  if (loading) return <ActivityIndicator size="large" color="#5D8AB0" style={styles.loader} />;
  if (error) return <Text style={styles.error}>{error}</Text>;

  if (!baby) {
    return <Text style={styles.errorText}>Không tìm thấy thông tin trẻ.</Text>;
  }

  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Text style={styles.header}>Hồ Sơ Trẻ</Text>
      <View style={styles.infoContainer}>
        <Text style={styles.infoText}><Text style={styles.label}>Tên trẻ: </Text>{baby.childrenFullname}</Text>
        <Text style={styles.infoText}><Text style={styles.label}>Ngày sinh: </Text>{new Date(baby.dob).toLocaleDateString()}</Text>
        <Text style={styles.infoText}><Text style={styles.label}>Giới tính: </Text>{baby.gender}</Text>
        <Text style={styles.infoText}><Text style={styles.label}>Tên cha: </Text>{baby.fatherFullName}</Text>
        <Text style={styles.infoText}><Text style={styles.label}>Tên mẹ: </Text>{baby.motherFullName}</Text>
        <Text style={styles.infoText}><Text style={styles.label}>SĐT cha: </Text>{baby.fatherPhoneNumber}</Text>
        <Text style={styles.infoText}><Text style={styles.label}>SĐT mẹ: </Text>{baby.motherPhoneNumber}</Text>
        <Text style={styles.infoText}><Text style={styles.label}>Địa chỉ: </Text>{baby.address}</Text>
      </View>
    </ScrollView>
  );
}
